...
function createJustfile ()
{
  ...
  echo "    podman run --network host -it -v \$PWD/project:/home/alpineuser/project --rm user/sandbox:${imageName}" >> .justfile 2> /dev/null
...
echo -e "${green}  docker run --network host -it -v "$PWD/project:/home/alpineuser/project" --rm user/sandbox:${imageName}"
